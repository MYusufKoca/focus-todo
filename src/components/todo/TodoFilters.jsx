...
const selectClassName = 'w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-600 outline-none focus:border-forest focus:ring-3 focus:ring-mint dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200'
  ...
  return (
    <section aria-label="Görevleri ara ve filtrele">
      <div className="flex gap-2 overflow-x-auto pb-2 lg:hidden">
        <button type="button" onClick={() => onFilterChange('category', 'all')} aria-pressed={filters.category === 'all'} className={`shrink-0 rounded-full px-4 py-2 text-sm font-medium ${filters.category === 'all' ? 'bg-forest text-white' : 'border border-slate-200 bg-white text-slate-600 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200'}`}>Tümü</button>
        {categoryOptions.map((option) => <button key={option.value} type="button" onClick={() => onFilterChange('category', option.value)} aria-pressed={filters.category === option.value} className={`shrink-0 rounded-full px-4 py-2 text-sm font-medium ${filters.category === option.value ? 'bg-forest text-white' : 'border border-slate-200 bg-white text-slate-600 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200'}`}>{option.label}</button>)}
      </div>
