...
const fieldClassName = 'w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 outline-none transition focus:border-forest focus:ring-3 focus:ring-mint dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100'
...
export default function TodoForm({ onSubmit, onCancel, initialValues, mode = 'create' }) {
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit} noValidate>
      ...
      <div><label htmlFor="todo-description" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">Açıklama <span className="font-normal text-slate-400">(isteğe bağlı)</span></label><textarea id="todo-description" name="description" rows="3" value={form.description} onChange={updateField} className={`${fieldClassName} resize-none`} placeholder="Görevle ilgili kısa bir not ekle..." /></div>
      <div className="grid gap-4 sm:grid-cols-2">
        <div><label htmlFor="todo-category" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">Kategori</label><select id="todo-category" name="category" value={form.category} onChange={updateField} className={fieldClassName}>{categoryOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>
        <div><label htmlFor="todo-priority" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">Öncelik</label><select id="todo-priority" name="priority" value={form.priority} onChange={updateField} className={fieldClassName}>{priorityOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>
      </div>
