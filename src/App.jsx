...
import { filterTodos, hasActiveFilters, initialFilters, normalizeTodos, sortTodos } from './utils/todoHelpers.js'

const normalizeTheme = (value, fallback) => ['light', 'dark'].includes(value) ? value : fallback

export default function App() {
  const [todos, setTodos] = useLocalStorage('focus-todo-tasks', [], normalizeTodos)
  const [theme, setTheme] = useLocalStorage('focus-todo-theme', () => window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light', normalizeTheme)
  ...
  const openCreateModal = (event) => {
    modalTriggerRef.current = event?.currentTarget ?? document.activeElement
    setEditingTodo(null)
    setIsModalOpen(true)
  }
  const openEditModal = (todo, trigger) => {
  ...
  const counts = useMemo(() => todos.reduce((result, todo) => ({
    active: result.active + Number(!todo.completed),
    completed: result.completed + Number(todo.completed),
    overdue: result.overdue + Number(isTodoOverdue(todo)),
  }), { active: 0, completed: 0, overdue: 0 }), [todos])
  ...
  const stats = [
    ...
    { label: 'Aktif görev', value: counts.active, accent: 'text-forest dark:text-emerald-300' },
    { label: 'Tamamlanan', value: counts.completed, accent: 'text-sky-700 dark:text-sky-300' },
    { label: 'Süresi geçen', value: counts.overdue, accent: 'text-red-600 dark:text-red-300' },
  ]

  return (
    <><div className="min-h-screen overflow-x-hidden lg:flex"><Sidebar todos={todos} /><div className="min-w-0 flex-1"><Header onNewTodo={openCreateModal} theme={activeTheme} onToggleTheme={toggleTheme} /><main id="main-content" className="mx-auto max-w-6xl px-5 py-7 md:px-8 md:py-10">
      ...
      <TodoFilters filters={filters} onFilterChange={updateFilter} onClearFilters={clearFilters} showClearButton={filtersAreActive} /><TodoList todos={filteredTodos} hasTodos={todos.length > 0} onNewTodo={openCreateModal} onToggle={toggleTodo} onDelete={deleteTodo} onEdit={openEditModal} />
    </main></div></div><div className="pointer-events-none fixed inset-x-4 bottom-5 z-[60] flex justify-center" aria-live="polite" aria-atomic="true"><div role="status" className={`rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-xl transition dark:bg-emerald-100 dark:text-emerald-950 ${notification ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}`}>{notification}</div></div><Modal isOpen={isModalOpen} title={editingTodo ? 'Görevi düzenle' : 'Yeni görev oluştur'} onClose={closeModal} returnFocusRef={modalTriggerRef}><TodoForm key={editingTodo?.id ?? 'new'} mode={editingTodo ? 'edit' : 'create'} initialValues={editingTodo} onSubmit={editingTodo ? updateTodo : addTodo} onCancel={closeModal} /></Modal></>
  )
}
